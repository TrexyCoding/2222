...
function update() {
  ...
  request(url, function(err, response, body) {
      ...
      var status = ' ' + body.players + " Kişi DREAMCRAFT'TA";
      ...
      if(body.players) {
          if((body.description=="&cWe are under maintenance.")||(body.players>=body.max_players)){
            ...
            if(body.players) {
                status = ' ' + body.players + " Kişi DREAMCRAFT'TA";
                client.channels.get(`834363666022531114`).setName('• Aktif: '+ body.players + '/'+ body.max_players);
              } else {
                status = ' 0/' + body.max_players;
                client.channels.get(`834363666022531114`).setName('• Aktif: '+ 0 + '/'+ body.max_players);
        }
      } else {
